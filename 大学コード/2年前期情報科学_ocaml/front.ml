(*？native_token-identifier (0)flush stdout、*)

module Lexer = struct

  
  type token = LET | BE | IN | EOF | ID of string | NUM of int | ONE of char
  module P = Printf (*openしないで使いたいときのため省略*) 
  exception End_of_system  
(* デフォルト入力ファイル *)
  let _ISTREAM = ref stdin (*C言語みたいに変数を操作するためref,後に参照するためのstdin*)
  let ch = ref [] (*読み込んだ文字の暗記*)
  let read () = match !ch with [] -> input_char !_ISTREAM (*y読み込んだ１文字の暗記*)
                             | h::rest -> (ch := rest; h)(*hを出力し、chの更新*)
  let unread c = ch := c::!ch (*1文字戻す作業*)
  let lookahead () = try let c = read () in unread c; c with End_of_file -> '$'(*読み込まず次の文字の確認*)

(* 整数の認識 *)
  let rec integer i =
    let c = lookahead () in
    if (c >= '0' && c <= '9') then
      integer (10*i+(Char.compare (read ()) '0')) (*文字から整数に変換し、桁一つ上げる*)
    else i

(* 識別子の認識 *)
  and identifier id = (*idはできている文字列*)
    let c = lookahead () in
    if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (*英字かアンダースコアならばこの先にも名前が続くかの判断*)
        (c >= '0' && c <= '9') || c == '_') then
      identifier (id^(Char.escaped (read ()))) (*先読みした１文字をreadで読み込み、idにくっつける*)
    else id  (*Char.escapedは文字列に変更するもの*)

(* 字句解析本体 *)
  and native_token () =
    let c = lookahead () in
    if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c  (*英字かアンダースコアならID*)
                                                            == '_') then
      let id = identifier "" in
      match id with
        "let" -> LET (*意味を持つID の時の判別*)
      | "be" -> BE
      | "in" -> IN
      | _ -> ID (id)
    else if (c >= '0' && c <= '9') then NUM (integer 0)
    else ONE (read ())

(* ホワイトスペースの読飛ばし含む *)
  and gettoken () =
    try
      let token = native_token () in
      match token with
        ONE ' ' -> gettoken ()
      | ONE '\t' -> gettoken ()
      | ONE '\n' -> gettoken ()
      | _ -> token
    with End_of_file -> EOF
  
(* トークンの印字 *)
  let print_token tk =
    match tk with
    | (ID i) -> P.printf "ID(%s)" i
    | (NUM n) -> P.printf "NUM(%d)" n
    | (LET) -> P.printf "LET"
    | (BE) -> P.printf "BE"
    | (IN) -> P.printf "IN"
    | (EOF) -> P.printf "EOF"
    | (ONE c) -> P.printf "ONE(%c)" c
                 
(* お試し実行関数 *)
  let rec run () =
    flush stdout;
    let rlt = gettoken () in
    match rlt with
      (ONE '$') -> raise End_of_system
    | _ -> (print_token rlt; P.printf "\n"; run())
    
end

module Ast = struct
  type definition = Def of string * expr * expr | Expr of expr (*let x be 1+2 inx+4*)
  and expr = Num of int | Var of string | App of expr * expr | Pair of expr * expr
end (*Appは（演算子,演算数）、例Expr(App ( Var "+" , pair(Num1 , Num2) ) ) *)




module Parser = struct 

  (* ストラクチャ名の省略 *)
  module L = Lexer 
  module A = Ast 
  (* 新しいトークンを取得し，tok に入れる *)
  let getToken () = L.gettoken ()
  let tok = ref (L.ONE ' ') (*字句解析機送られてきたトークンを覚えておく*)
  let advance () = tok := getToken()  (*tokの中身を変える*)
(* let advance () = (tok := getToken(); L.print_token (!tok)) *)
(* エラー出力 *)
  exception Syntax_error
  let error () = raise Syntax_error
(* 特定のトークンが来ているかチェックする *)
  let eat t = if (!tok=t) then advance() else error()
  let eatID () = match !tok with (L.ID id) -> (advance(); id)
                               | _ -> error()
  let eatNUM () = match !tok with (L.NUM num) -> (advance(); num)
                                | _ -> error()
(* 文法チェック *)
  let rec parse () = (advance(); p())
  and p () = match !tok with L.LET -> (eat(L.LET);
                                       let _str = eatID() in (eat(L.BE);
                                                              let _e1 = e() in eat(L.IN);
                                                              let _e2 = e() in (eat(L.ONE ';');
                                                                                A.Def (_str, _e1, _e2))))
                           | _ -> let _e = e() in (eat(L.ONE ';'); A.Expr _e)
  and e () = let _t = t() in (e' _t)
  and e' _e = match !tok with L.ONE '+' -> (eat(L.ONE '+');
                                            let _pre = A.App(A.Var "+", A.Pair(_e, t())) in (e' _pre))
                            | L.ONE '-' -> (eat(L.ONE '-');
                                            let _pre = A.App(A.Var "-", A.Pair(_e, t())) in (e' _pre))
                            | _ -> _e
  and t () = let _f = f() in (t' _f)
  and t' _t = match !tok with L.ONE '*' -> (eat(L.ONE '*');
                                            let _pre = A.App(A.Var "*", A.Pair(_t, f())) in (t' _pre))
                            | L.ONE '/' -> (eat(L.ONE '/');
                                            let _pre = A.App(A.Var "/", A.Pair(_t, f())) in (t' _pre))
                            | _ -> _t
  and f() = match !tok with
      L.ID _ -> A.Var (eatID())
    | L.NUM _ -> A.Num (eatNUM())
    | _ -> error()

end ;;





