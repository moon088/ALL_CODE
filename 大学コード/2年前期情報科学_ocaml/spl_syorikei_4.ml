...
(*追加*) 
  ...
  let rec  print_succeed lst=  (*変更*)
    ...
    |(Atom s)::rest -> let ex1 () = P.printf (",%s") s; print_succeed rest and ex2 () = P.printf ("%s ; ")s; print_succeed rest in
        if !count_of_var = 0 then ex1 () else ex2 ()
    | _ -> ()
  ...
  let rec succeed query = match !save_hensuu with (*変更*)
    |(Var "") -> 
        let rec succeed' query = match query with
          |App (_ , []) ->  () ; 
          |App (predicate , [Atom s]) -> let ex1 () = P.printf ("%s") predicate; P.printf "(" ; P.printf ("%s)") s 
              and ex2 () = P.printf ("%s ; ") s in
              if !count_of_var = 0 then ex1 () 
              else ex2 ()
          |App (predicate , (Atom s)::rest ) -> let ex0 () = P.printf ("%s ; ") s ; print_succeed rest and ex0' () = P.printf ""; print_succeed rest in
    ...
    |(Var  n) -> (match test_variable n with  
        ...
        |false -> P.printf ("%s = ") n; save_hensuu' := n; save_hensuu := (Var ""); count_of_var := 1;  succeed query    ) 
    | _ -> false 
